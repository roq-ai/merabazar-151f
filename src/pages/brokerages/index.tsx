...
type ColumnType = ColumnDef<BrokerageInterface, unknown>;

interface BrokerageListPageProps {
...
export function BrokerageListPage(props: BrokerageListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<BrokerageInterface>>(
    () => `/brokerages?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: BrokerageInterface) => {
    if (hasAccess('brokerage', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/brokerages/view/${row.id}`);
    }
  };
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/brokerages/view/${record.id}`} passHref legacyBehavior>
            <Button
